...
export function TileLayout({
  ...
  const remoteUnknownTracks = useTracks([Track.Source.Unknown]);

  // 1) On exclut toujours le participant local
  const nonLocalRemoteTracks = [...remoteCameraTracks, ...remoteUnknownTracks].filter(
    (trackRef) => trackRef.participant.identity !== localParticipant.identity
  );

  // 2) On cherche d'abord explicitement l'avatar Anam
  const anamAvatarTrack = nonLocalRemoteTracks.find(
    (trackRef) => trackRef.participant.identity === 'anam-avatar-agent'
  );

  // 3) Fallback : si l'identité change un jour, on prend quand même une piste distante non locale
  const fallbackRemoteTrack = nonLocalRemoteTracks[0];

  // 4) Priorité : avatar Anam > autre piste distante > voice assistant
  const agentVideoTrack = anamAvatarTrack ?? fallbackRemoteTrack ?? voiceAssistantVideoTrack;
